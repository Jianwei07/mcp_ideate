...
import type { CitationValidation } from "./retrieval/citations.ts";
...
export function citationRepairPrompt(
  ...
  const diagnostics = [
    `has citations: ${validation.hasCitations}`,
    `claim-like sentence count: ${validation.claimCount}`,
    `uncited claim-like sentence count: ${validation.uncitedClaimCount}`,
    `invalid source markers: ${validation.invalidSourceIds.join(", ") || "none"}`,
  ].join("\n");
...
Rewrite the answer as valid JSON. Every sentence in answer must include at least one valid source marker from the approved evidence, such as [S1]. If using bullets, every bullet must include a valid source marker. Do not cite sources that are not listed above. If the answer cannot be fully grounded, return status "insufficient_evidence".`;
}
